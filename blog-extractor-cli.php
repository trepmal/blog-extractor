	...
	function blog( $args, $assoc_args ) {
		...
		if ( ! isset( $assoc_args['exclude-archive'] ) ) :
			...
			$plugins = array_map( function($i) {
				...
				// plugins listed in the options table does not promise existence
				if ( file_exists( WP_CONTENT_DIR .'/plugins/'. $i ) ) {
		...
		if ( $v ) {
			...
			WP_CLI::line( "tar -cvf {$export_file} {$exports} {$exclude}" );
		}
